...
function positionIteration_superAttractingFixedPoints(coefficientlistnum::Array{Complex{Float64},1},coefficientlistden::Array{Complex{Float64},1},
  ...
  # Pre: maxcycle>=1
  # Post: Returns a matrix of vectors with 3 components (position, iteration and Lyapunov constant),
  #       the list of super-attracting fixed points and whether if infinity is a fixed point.
  if inverted==true
  ...
  positer=Array{Vector{Float64}}(undef,a,b)
  @inbounds Threads.@threads for j in 1:b
    @inbounds for i in 1:a
      ...
      if infFixedPoint==true && length(cycle[1])==1 && abs(cycle[1][1][2])<tol # In case the orbit of the point converges to infinity.
        positer[i,j]=[1.0,cycle[2],cycle[3]] # Note that, if infinity is a fixed point, the position 1 on the list of fixed points is reserved to it.
      elseif cycle[3]<tol && length(cycle[1])==1 && abs(cycle[1][1][2])>tol # In case the orbit of the point converges to another super-attracting fixed point.
        ...
        if pos[1]==false
          ...
          positer[i,j]=[length(fixedPoints),cycle[2],cycle[3]]
        else
          positer[i,j]=[pos[2],cycle[2],cycle[3]]
        end
      else # In case the orbit of the point does not converge to any point (1-cycle).
        positer[i,j]=[0.0,cycle[2],cycle[3]]
      end
    end
  end
...
function colorRectangle_positionIteration(coefficientlistnum::Array{Complex{Float64},1},coefficientlistden::Array{Complex{Float64},1},
  ...
  elseif coloringStrat=="position"
    ...
    colorMatrix[1,1]=0.0 # You might need to remove these 2 lines in order to save the raw image.
    colorMatrix[a,b]=numFixedPoints
  elseif coloringStrat=="lyapunov"
    @inbounds colorMatrix=[positer[1][i,j][3] for i=1:a,j=1:b]
    @inbounds Threads.@threads for j in 1:b
      @inbounds for i in 1:a
        if colorMatrix[i,j]<tol
          colorMatrix[i,j]=0
        elseif colorMatrix[i,j]<1.0
          colorMatrix[i,j]=floor(255*(colorMatrix[i,j]))
        else
          colorMatrix[i,j]=255
        end
      end
  ...
  if coloringStrat!="lyapunov"
    ...
    img=PyPlot.imshow(colorMatrix,cmap="gray",extent=[xinterval[1],xinterval[2],yinterval[1],yinterval[2]],vmin=0,vmax=255)
    gseed=0
    numColors=0
  end
  return positer[2],gseed,img,numColors,colorMatrix
end
...
function plotBasinsOfAttraction_Lyapunov(coefficientlistnum::Array{Complex{Float64},1},coefficientlistden::Array{Complex{Float64},1},
  ...
  #       Also returns the number of plotted colors, the
  #       positer matrix and the seed used to color it.
  ...
  return coloredRect[3], coloredRect[4], coloredRect[5], coloredRect[2] # Returns the resulting image, the number of colors used, an the positer matrix.
end
...
function plotWithCycles_BasinsOfAttraction_Lyapunov(coefficientlistnum::Array{Complex{Float64},1},coefficientlistden::Array{Complex{Float64},1},
  ...
  for j in 1:numCycles
    ...
    for i in 1:numElements
      ...
      if inverted==true
        x_values=[x1/(x1^2+y1^2),x2/(x2^2+y2^2)]
        y_values=[-y1/(x1^2+y1^2),-y2/(x2^2+y2^2)]
      else
        x_values=[x1,x2]
        y_values=[y1,y2]
      end
  ...
  return img, numColors, colorMatrix, gseed
end
...
function detectCathastrophicBehaviour_cubicPolynomials_Newton(xinterval::Tuple{Float64,Float64},yinterval::Tuple{Float64,Float64},
  ...
  posMatrix[1,1]=0 # You might need to remove these 2 lines in order to save the raw image.
  posMatrix[a,b]=4
  ...
  return img, numColors, posMatrix, gseed
end
...
function detectCathastrophicBehaviour_cubicPolynomials_Chebyshev(xinterval::Tuple{Float64,Float64},yinterval::Tuple{Float64,Float64},
  maxiterations::Int64=25,maxcycle::Int64=5,tolerance::Int64=8)
  # Pre:
  # Post: Plots the image, and returns the number of plotted colors.
  rect=rectangle(xinterval,yinterval)
  ...
  @inbounds Threads.@threads for j in 1:b
    @inbounds for i in 1:a
      lambda=rect[i,j][1]
      coefficientlistnum=complex([lambda+lambda^3,-9.0*lambda^2,6.0*(2.0*lambda+lambda^3),-1.0*(1.0+18.0*lambda^2),3.0*(3.0*lambda-lambda^3),3.0*(2.0+5.0*lambda^2),-26.0*lambda^2,15.0]) # Constructs the rational function given by Chebyshev's method applied to the polynomial associated with this particular point. 
      coefficientlistden=complex([1.0,-6.0*lambda,3.0*(3.0+4.0*lambda^2),-4.0*(9.0*lambda+2.0*lambda^3),9.0*(3.0+4.0*lambda^2),-54.0*lambda,27.0])
      cycle=cycleDetector_Lyapunov(coefficientlistnum,coefficientlistden,(lambda,complex(3.0)),maxiterations,maxcycle,tolerance)
      cycleConvergence=false
      if length(cycle[1])>1
        cycleConvergence=true
      else
        cycle2=cycleDetector_Lyapunov(coefficientlistnum,coefficientlistden,(-10.0*lambda+sqrt(15.0-5.0*lambda^2),complex(-15.0)),maxiterations,maxcycle,tolerance)
        if length(cycle2[1])>1
          cycleConvergence=true
        else
          cycle3=cycleDetector_Lyapunov(coefficientlistnum,coefficientlistden,(-10.0*lambda-sqrt(15.0-5.0*lambda^2),complex(-15.0)),maxiterations,maxcycle,tolerance)
          if length(cycle3[1])>1
            cycleConvergence=true
          end
        end
      end
      if cycleConvergence==false && length(cycle[1])==1 && abs(cycle[1][1][2])>tol # In case the orbit of the point converges to a fixed point other than the infinity point.
        if cycle[3]<tol # In case the fixed point is super-attracting.
          if abs(cycle[1][1][1]/cycle[1][1][2]-1.0*im)<tol # Checks if said point is im.
            posMatrix[i,j]=1.25
          elseif abs(cycle[1][1][1]/cycle[1][1][2]+1.0*im)<tol # Checks if said point is -im.
            posMatrix[i,j]=2.25
          elseif abs(cycle[1][1][1]/cycle[1][1][2]-lambda)<tol # Checks if said point is lambda.
            posMatrix[i,j]=3.25
          else # If not, the fixed point is extraneous.
            posMatrix[i,j]=4.25
          end
        else # If the fixed point is not super-attracting, it is extraneous.
          posMatrix[i,j]=4.25
        end
      else # In case the orbit of the point does not converge to any root of the polynomial (1-cycle).
        posMatrix[i,j]=0.25
  ...
  @inbounds Threads.@threads for j in 1:b
    @inbounds for i in 1:a
      ...
      if cycle[3]<tol # In case the orbit of the point converges to a super-attracting fixed point (a root of the polynomial) or a super-attracting n-cycle.
        posMatrix[i,j]=255
      elseif cycle[3]<1.0
        posMatrix[i,j]=255-floor(255*(cycle[3]))
      else
        posMatrix[i,j]=0
      end
    end
  end
  img=PyPlot.imshow(posMatrix,cmap="gray",extent=[xinterval[1],xinterval[2],yinterval[1],yinterval[2]],vmin=0,vmax=255)
  return img, posMatrix
end
